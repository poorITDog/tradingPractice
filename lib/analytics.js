...
export function styleTips(dims) {
  ...
  const ranked = Object.entries(dims)
    ...
    .sort((a, b) => a[1] - b[1]);
  const tipsMap = {
    trendFollow: '試喺價喺 1h MA(20) 上方先考慮做多，減少逆勢衝動。',
    meanRevert: '區間單要縮小槓桿，並設定明確失效位。',
    riskControl: '每單預設 Stop Loss；單筆風險控制喺權益 1–2%。',
    discipline: '開倉前同時掛 TP 與 SL，避免無計畫持倉。',
