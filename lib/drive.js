...
export function mergeDriveState(local, remote) {
  ...
  // Union equity samples by timestamp; keep denser history.
  const sampleMap = new Map();
  for (const s of [...(older.equitySamples || []), ...(newer.equitySamples || [])]) {
    if (!s || s.t == null) continue;
    sampleMap.set(s.t, s);
  }
  const equitySamples = [...sampleMap.values()].sort((a, b) => a.t - b.t).slice(-4000);

  // Union fills by id so funding history survives sync.
  const fillMap = new Map();
  const olderFills = older.account?.fills || [];
  const newerFills = newer.account?.fills || [];
  for (const f of [...olderFills, ...newerFills]) {
    if (f?.id) fillMap.set(f.id, f);
  }
  const account = newer.account
    ? {
      ...newer.account,
      fills: fillMap.size
        ? [...fillMap.values()].sort((a, b) => (a.ts || 0) - (b.ts || 0))
        : (newer.account.fills || []),
    }
    : newer.account;
