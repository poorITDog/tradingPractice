...
export function placeOrder(account, input, ctx) {
  const {
    ...
    reduceOnly = false, tp, sl, triggerPrice, tif = 'GTC', trailPct,
  } = input;
  ...
  if (ordType === 'stop_trail') {
    const trail = Number(trailPct);
    if (!(trail > 0 && trail < 50)) return { ok: false, reason: 'trail' };
    if (mark == null || !(mark > 0)) return { ok: false, reason: 'mark' };
    const order = {
      id: id('ord'),
      symbol,
      side,
      ...
      trailPct: trail,
      peak: mark,
      triggerPrice: side === 'short' ? mark * (1 - trail / 100) : mark * (1 + trail / 100),
      leverage,
...
export function maybeFillLimits(account, market, fees) {
  ...
  for (const o of account.orders) {
    ...
    if (o.ordType === 'stop_trail') {
      if (last == null || !(o.trailPct > 0)) continue;
      // Sell trail (short): track high, fill on pullback. Buy trail (long): track low, fill on bounce.
      if (o.side === 'short') {
        if (last > (o.peak || 0)) {
          o.peak = last;
          o.triggerPrice = last * (1 - o.trailPct / 100);
        }
        if (last <= o.triggerPrice) {
          o.status = 'filled';
          results.push(fillOrder(account, {
            symbol: o.symbol, side: o.side, qty: o.qty, price: last,
            leverage: o.leverage, reduceOnly: o.reduceOnly, tp: o.tp, sl: o.sl,
            feeRate: fees.taker, liquidity: 'taker', mark, reason: 'trail',
          }, { marks: { [o.symbol]: mark } }));
        }
      } else {
        if (o.peak == null || last < o.peak) {
          o.peak = last;
          o.triggerPrice = last * (1 + o.trailPct / 100);
        }
        if (last >= o.triggerPrice) {
...
export function accountSnapshot(account, marks, fees) {
  ...
  const used = usedMarginMicros(account, marks);
