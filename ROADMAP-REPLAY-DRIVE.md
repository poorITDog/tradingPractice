...
| Phase | 內容 | 狀態 |
|-------|------|------|
| U | 倉位列平倉／反手／改 TP·SL／部分平倉／撤單 | ✅ 交易所級 blotter |
| D0 | `lib/drive.js` + 設定頁連接 | ✅ 骨架已做（`apex-trade-lab-v1.json`） |

### Post-U CEX 對照（Similarity 驗收）

| 控制項 | 狀態 |
|--------|------|
| 逐倉／全倉選擇器 | ✅ 逐倉；全倉 disabled（誠實 stub） |
| 單向／雙向 | ✅ 單向；雙向 disabled |
| 市價／限價／條件市價／條件限價 | ✅ |
| TIF GTC／IOC／PostOnly | ✅ |
| 可開多空、幣／USDT 數量、ROI% TP/SL 快捷 | ✅ |
| 列內平倉％／反手／改TP·SL／調保證金±／調槓桿 | ✅ |
